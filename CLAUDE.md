...
- **仓库**: `https://github.com/FrichXi/funeral-ai-web4`（public，分支 `main`）
...
npm run deploy                                        # 部署（默认禁用本机代理变量）
```

部署脚本定义见 `site/package.json`：
- `npm run deploy`: 先从仓库根目录运行 `python3 -m scripts.run_pipeline build` 和 `python3 scripts/kg_review_gate.py`，再执行 `npm run build`，最后清掉 `ALL_PROXY` / `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY` 的大小写变量并执行 `wrangler pages deploy out --project-name funeral-ai-web4`
